...
if __name__ == '__main__':
    ...
    for index, row in df_dummy.iterrows():
        df_full.append({
            ...
            "Dummy_dbpedia18": row["Dummy_dbpedia18"],
            "template_id_dummy_dbpedia18": row["Dummy_id_wikidata"],
            "template_id_dummy_group_dbpedia18": math.floor(row["Dummy_id_wikidata"]),
    ...
    pd.DataFrame(df_full.question.str.len()).boxplot().figure.savefig("./boxplot_antes.png")
    ...
    df_train.to_json("./training_without_paraphrased.json", orient="records", default_handler=str)
    df_test.to_json("./testing.json", orient="records", default_handler=str)
    df_valid.to_json("./dev.json", orient="records", default_handler=str)
    ...
    df_train_parafrase.to_json("./training_with_paraphrased.json", orient="records", default_handler=str)
