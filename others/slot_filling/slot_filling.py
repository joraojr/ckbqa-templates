...
if __name__ == "__main__":
    ...
    with open(os.path.join(qald_dir, 'falcon_entities_train_wikidata.json'), 'r') as inputfile:
        ...
        for item in prediction:
            item_template = []
            # for template_id in item["predictions"]:
            #    item_template.append({"id": template_id, "template": templates[templates.old_id == template_id][
            #        "Template_Dummy_Wikidata"].item()})
            # json_object.append({
            #    'question': item["question"],
            #    'sparql': item["sparql"],
            #    'sparql_dummy': item["sparql_dummy"],
            #   # 'prediction': item_template,
            #    'wikidata_entities': get_wikidata_entites(item["question"]),
            # })
