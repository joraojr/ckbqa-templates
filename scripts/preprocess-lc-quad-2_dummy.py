...
if __name__ == '__main__':
    ...
    lc_quad_dir = os.path.join(data_dir, 'lc-quad-2-dummy')
    ...
    df_dummy = pd.read_json(os.path.join(lc_quad_dir, "lcquad_2_dummys.json"))
    df_train = pd.read_json(os.path.join(lc_quad_dir, "train.json"))
    df_test = pd.read_json(os.path.join(lc_quad_dir, "test.json"))

    for index, row in df_train.iterrows():
        dummy_db = df_dummy[df_dummy["uid"] == row["uid"]]["Dummy_dbpedia18"]
        df_train.loc[index, "Dummy_dbpedia18"] = dummy_db.values[0].strip()

    for index, row in df_test.iterrows():
        dummy_db = df_dummy[df_dummy["uid"] == row["uid"]]["Dummy_dbpedia18"]
        df_test.loc[index, "Dummy_dbpedia18"] = dummy_db.values[0].strip()
    ...
    desired_templates = pd.read_csv(os.path.join(lc_quad_dir, "templates_frequencia.csv"))
    print(desired_templates["Dummy Template dbpedia18"])

    def to_template_index(row):
        x = desired_templates.loc[
            desired_templates["Dummy Template dbpedia18"] == row['Dummy_dbpedia18'], "id"].index.item()
        return x
